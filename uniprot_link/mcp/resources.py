...
UNIPROT_SERVER_INSTRUCTIONS = (
    ...
    "ec_number=, mnemonic=, name_contains=) searches UniProtKB for a gene's entries -- "
    "gene_symbol is REQUIRED; refine with the optional filters. organism_taxon is an "
    ...
    "-- it resolves them concurrently in one call and reports unresolved_genes. For "
    "EC-only / keyword-only / free-text search without a gene, use search_sparql_query.\n"
    ...
    "`evidence_codes`. Every tool returns structured output (structuredContent); "
    "outputSchema is intentionally not published (Tool-Surface Budget).\n"
    ...
    "- Verbosity: get_protein / get_protein_sequence / get_protein_features / "
    ...
    "Defaults are per-tool: the annotation tools retain standard/full records unless "
    "you request compact/minimal, which removes repeated fenced prose while retaining "
    "record identifiers. get_protein_sequence compact returns a first/last-30-residue "
    "preview; use standard/full for the complete sequence.\n"
...
UNIPROT_USAGE_NOTES = (
    ...
    "(needs a gene_symbol; optionally an organism taxon id, reviewed flag, or filters) "
    "or get_taxon. "
